...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # h1 = self.params['W1'].dot(x_flat) + self.params['b1']
    # scores = self.params['W2'].dot(h1) + self.params['b2']
    # reg = self.reg

    h1, cache = affine_forward(X, self.params['W1'], self.params['b1'])
    relu, cache = relu_forward(h1)
    scores, cache = affine_forward(relu, self.params['W2'], self.params['b2'])
    ...
    #   'grads' dictionary.  For the grads dictonary, grads['W1'] holds
    ...
    grad_h2 = grad_softmax
    grad_relu = grad_h2.dot(W2.T)
    grad_h1 = relu_backward(grad_relu, (h1))

    dx2, dw2, db2 = affine_backward(grad_h2, (relu, W2, b2))
    grads['W2'] = dw2 + reg * W2
    grads['b2'] = db2
    dx1, dw1, db1 = affine_backward(grad_h1, (X, W1, b1))
    grads['W1'] = dw1 + reg * W1
    grads['b1'] = db1
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(num_layers-1): # no relu in last layer
      prev, cached[i] = affine_relu_forward(prev, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
    ...
    for i in reversed(range(num_layers-1)): # relu -> affine ->
      ...
      dx, grads[widx], grads[bidx] = affine_relu_backward(dout, cached[i])
