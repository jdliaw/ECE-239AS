...
def adam(w, dw, config=None):
  ...
  config['a'] = beta2 * config['a'] + (1-beta2) * dw * dw
  v_u = config['v'] / (1 - beta1 ** config['t'])
  a_u = config['a'] / (1 - beta2 ** config['t'])
  next_w = w - ((eps * v_u) / (np.sqrt(a_u) + nu))
