...
def affine_forward(x, w, b):
  ...
  x_reshaped = np.reshape(x, (x.shape[0], w.shape[0]))
...
def affine_backward(dout, cache):
  ...
  x_reshaped = np.reshape(x, (x.shape[0], w.shape[0]))
  dx_t = dout.dot(w.T)
  dx = np.reshape(dx_t, x.shape)
  dw = (dout.T.dot(x_reshaped)).T
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    xhat = (x - np.mean(x, axis=0)) / np.sqrt(np.var(x, axis=0) + eps)
    out = gamma * xhat + beta

    mu = np.mean(x, axis=0)
    var = np.var(x, axis=0)
    cache = (x, gamma, eps, mu, var, xhat)
    ...
    x = (x - bn_param['running_mean']) / np.sqrt(bn_param['running_var'] + eps)
    out = gamma * x + beta
...
def batchnorm_backward(dout, cache):
  ...
  m = dout.shape[0]
  x, gamma, eps, mu, var, xhat = cache

  dxhat = dout * gamma
  dmu = - (1 / np.sqrt((var) + eps)) * np.sum(dxhat, axis=0)
  dvar = np.sum((-0.5 * (x - mu) * dxhat) / (((var) + eps) ** (3/2)), axis=0)
  dx = (dxhat / np.sqrt(var + eps)) + ((2 * (x - mu)) / m) * dvar+ (dmu / m)
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    mask = np.random.rand(*x.shape) < p # generate mask
    out = x * mask
