...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    #   training examples.)
    # ================================================================ #
      predicted = X[i].dot(self.W.T)
      actual = predicted[y[i]]

      for j in np.arange(num_classes):
        if j == y[i]:
          continue
        margin = predicted[j] - actual + 1
        if margin > 0:
          loss += margin
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    margin = np.maximum(0, scores - np.matrix(yi_scores).T + 1)
    ...
    binary = margin
    binary[margin > 0] = 1
    rsum = np.sum(binary, axis=1)
    binary[np.arange(num_train), y] = -rsum.T
    grad = np.dot(X.T, binary).T
    grad /= num_train
  ...
  def train(self, X, y, learning_rate=1e-3, num_iters=100,
    ...
    for it in np.arange(num_iters):
      ...
      idx = np.random.choice(num_train, batch_size) # get batch_size amt of rand num
      X_batch = X[idx]
      y_batch = y[idx]
